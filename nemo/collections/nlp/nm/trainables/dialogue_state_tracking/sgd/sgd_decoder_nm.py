...
class SchemaMultiAttention(nn.Module):
    ...
    def forward(self, encoded_utterance, token_embeddings, element_embeddings):
        ...
        if mask is not None:
            ...
            new_x_shape = (mask.size()[0], mask.size()[1]) + (1, 1) + (mask.size()[-1],)
            attention_scores = torch.where(mask.view(*new_x_shape).to(bool), attention_scores, negative_scores)
...
class SchemaAttention(nn.Module):
    ...
    def forward(self, encoded_utterance, token_embeddings, element_embeddings, utterance_mask=None):
        """
        token_embeddings - token hidden states from BERT encoding of the utterance (batch_size, seq_len, embedding_dim)
        encoded_utterance - [CLS] token hidden state from BERT encoding of the utterance (batch_size, embedding_dim)
        element_embeddings: A tensor of shape (batch_size, num_elements, seq_len, embedding_dim).
...
class SGDDecoderNM(TrainableNM):
    ...
    def __init__(
        self, embedding_dim, schema_emb_processor, add_attention_head=False, attention_schema=False, **kwargs
    ):
        """Get logits for elements by conditioning on utterance embedding.
        ...
        if attention_schema:
            projection_module = SchemaAttention
    ...
    def forward(
        ...
        if self.attention_schema:
            cat_slot_value_emb = self.cat_slot_value_emb(service_ids).view(
                batch_size, max_number_cat_slots, -1, seq_len, emb_dim
            )
        else:
            cat_slot_value_emb = self.cat_slot_value_emb(service_ids).view(
    ...
    def _get_categorical_slot_goals(
        ...
        if cat_slot_emb.ndim == 3 and cat_slot_value_emb.ndim == 4:
            ...
            cat_slot_value_emb_reshaped = cat_slot_value_emb.view(
                -1, max_num_slots * max_num_values, seq_len, embedding_dim
            )
    ...
    def _get_noncategorical_slot_goals(self, encoded_utterance, utterance_mask, noncat_slot_emb, token_embeddings):
        ...
        if not self.attention_schema:
            ...
            span_logits = self.noncat_layer1(
                token_embeddings=token_embeddings,
                element_embeddings=noncat_slot_emb,
                encoded_utterance=encoded_utterance,
            )
